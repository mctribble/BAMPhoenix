 ...
 *         AssignForce
 *         and updates the BAM Batch, Users, and Batch_Type tables with the
 *         corresponding values.
 *         It utilizes 3 Objects, AssignForceBatch, AssignForceCurriculum, and
 *         AssignForceTrainer.
 *         These objects are used to hold data from a ResponseEntity, and are
 *         later used to
...
public class AssignForceSyncService {
  ...
  // Makes a call to the AssignForce API and builds POJOs from the returned JSON
  // String.
  static ResponseEntity<List<AssignForceBatch>> batches = restTemplate.exchange(baseUrl + "batch/", HttpMethod.GET,
      null, new ParameterizedTypeReference<List<AssignForceBatch>>() {
      });
  ...
  public void assignForceSync() {
    ...
    // Object necessary for creation of batch; populated with AssignForceBatch
    // data.
    ...
    for (int i = 0; i < batches.getBody().size(); i++) {
      ...
      if (batch.getTrainer() != null) {
        ...
        // Null check for curricula, assignment of curriculum values to
        // batch_type.
        ...
        // First and last names pulled from AssignForce trainer to search for
        // current trainers.
        ...
        // If List is not empty and user is a trainer, trainer is assigned to
        // batch.
        ...
        // Batch is persisted, the batch will only be added if it doesn't
        // already exist in db
