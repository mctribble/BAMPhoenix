...
app.controller("subTopicController", function ($scope, SessionService, $location, $compile, $http) {
    ...
    $scope.getSubtopicPool = function () {
        $http({
            ...
            var subtopicArray = [];
            for (i in subtopics) {
                if (!uniqueTopics.has(subtopics[i].topic.name)) {
                    uniqueTopics.add(subtopics[i].topic.name);

                    var array = [];
                    array.push(subtopics[i].name);
                    topicMap.set(subtopics[i].topic.name, array);
                } else {
                    var array = topicMap.get(subtopics[i].topic.name);
                    topicMap.delete(subtopics[i].topic.name);
                    array.push(subtopics[i].name);
                    topicMap.set(subtopics[i].topic.name, array);
                }
            }
            ...
            $('#topicname').change(function () {
                ...
                for (i in subtopics) {
                    if ($scope.selectedSubtopic == subtopics[i].name) {
                        $scope.topic_id = subtopics[i].topic.id;
                        $scope.subtopic_id = subtopics[i].id;
                    }
                }
            });
