...
public class CurriculumControllerTest {
	...
	public void getAllCurriculumTest() throws Exception{
		mockMvc.perform(get("/api/v1/Curriculum/All").requestAttr("bid", "1")).andExpect(status().isOk()).andExpect(content().contentType("application/json"));
	}
	...
	public void getScheduleTest() throws Exception{
		mockMvc.perform(get("/api/v1/Curriculum/Schedule")).andExpect(status().isOk());
	}
